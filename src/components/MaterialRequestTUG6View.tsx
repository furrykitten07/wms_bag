...
export default function MaterialRequestTUG6View({
  ...
  const handleSyncSPK = () => {
    ...
    if (unmatchedSPKs.length === 0) {
      alert(`Seluruh daftar SPK (${spkList.length} SPK) sudah tersinkronisasi sempurna dengan TUG 6 (Permintaan Barang) di sistem!`);
      return;
    }
    ...
    try {
      const payloads: Partial<MaterialRequest>[] = toSync.map(spk => {
        ...
        spk.vessels.forEach(vessel => {
          vessel.items.forEach(itm => {
            ...
            allItems.push({
              ...
              notes: `Sinkronisasi Otomatis SPK TUG 6 - Kapal: ${vessel.vessel_name}`
            });
          });
        });
        ...
        return {
          ...
          remarks: `SINKRONISASI OTOMATIS REGISTER SPK TUG 6: ${spk.spk_number}`,
      ...
      setPendingSyncList([]);
      setSelectedSyncIds([]);
      alert(`Sinkronisasi sukses! ${payloads.length} dokumen TUG 6 (Permintaan Barang) telah disinkronkan dengan status DRAFT. Harap setujui dengan akun Staff Gudang.`);
    } catch (err: any) {
      console.error(err);
  ...
  const handleApprove = async () => {
    if (!activeMR) return;
    if (confirm(`Approve Material Request ${activeMR.request_number} & generate TUG 6?`)) {
  ...
  const handleSubmitClick = async (mr: MaterialRequest) => {
    if (currentUser.role === UserRole.WAREHOUSE_ADMIN || currentUser.role === UserRole.SUPER_ADMIN) {
      if (confirm(`Approve Draft TUG 6 ${mr.request_number} (Status -> Submitted)?`)) {
        await onUpdateRequest(mr.id, { status: "Submitted" });
      }
    } else {
      alert(`Permintaan TUG 6 ${mr.request_number} telah diajukan ke Staff Gudang untuk diverifikasi!`);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 border-l border-slate-200">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1 font-sans font-medium">
            Sistem pengajuan logistik kapal PT. Pelayaran Bahtera Adhiguna. Ajukan, setujui, dan unduh form cetak TUG 6 resmi.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 shrink-0">
          ...
          >
            ...
            <span>Export ZIP Batch TUG 6</span>
          </button>
          ...
          <table className="w-full text-left border-collapse min-w-[1200px]">
            <thead>
              <tr className="bg-slate-50 text-slate-700 text-[10px] font-mono uppercase tracking-wider border-b border-slate-200 sticky top-0 z-10">
                ...
                <th className="py-4 px-6 font-semibold">Pemohon</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td 
                        ...
                        className="py-4.5 px-6 font-mono font-black text-indigo-600 hover:underline cursor-pointer"
                      >
                        {mr.request_number}
                      </td>
        ...
        <div className="fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto no-print">
          <div className="bg-white border border-slate-205 rounded-xl shadow-2xl max-w-4xl w-full font-sans overflow-hidden my-8 flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                <h4 className="text-[9px] font-black font-mono uppercase tracking-widest text-slate-400 mb-3 text-center">
                  TUG 6 MATERIAL REQUEST LIFECYCLE STATE LOG
                </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 leading-relaxed text-xs">
                ...
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-2.5">
                  ...
                  <div className="flex justify-between border-b border-slate-100 pb-1 font-mono text-[11px]">
                    ...
                    <span className="text-indigo-700 font-bold">{activeMR.request_number}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-100 pb-1 font-mono text-[11px]">
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-205 flex flex-col sm:flex-row justify-between items-center gap-3 shrink-0">
              ...
              {/* Left Action group: View/Print TUG 6 */}
              <div>
                <button
                  onClick={() => {
                    onPreviewTUG6(activeMR);
                  }}
              ...
              <div className="flex gap-2">
                ...
                {activeMR.status === "Draft" && currentUser.role === UserRole.VESSEL_CREW && (
                  <button
                    onClick={async () => {
                      if (confirm(`Submit Material Request TUG 6 ${activeMR.request_number}?`)) {
          ...
          <div className="bg-white border border-slate-205 rounded-xl shadow-2xl max-w-5xl w-full font-sans overflow-hidden my-8 flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 border-b border-slate-205 bg-slate-50 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-black font-display text-slate-950 tracking-wide uppercase">
                  {isCreating ? "Isi Data Permintaan Barang Baru (TUG 6)" : `Edit Dokumen TUG 6 — ${activeMR?.request_number}`}
                </h3>
              </div>
              <button
                onClick={() => {
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl space-y-4">
                <h4 className="text-[10px] font-black font-mono uppercase tracking-widest text-slate-500 border-b border-slate-200 pb-1.5 flex items-center gap-1">
                  <span>1. TUG 6 MATERIAL REQUISITION HEADER</span>
                </h4>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-slate-250 rounded-lg text-xs px-3 py-2 font-mono text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-slate-250 rounded-lg text-xs px-3 py-2 font-semibold text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-slate-250 rounded-lg text-xs px-3 py-2 font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 bg-blue-50/20 p-3 rounded-lg border border-blue-100/50">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white border border-slate-250 rounded-lg text-xs px-3 py-2 font-mono text-rose-600 font-bold focus:outline-none focus:ring-1 focus:ring-indigo-500 cursor-pointer"
                      >
                        ...
                        return (
                          <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-center">
                            ...
                            <button
                              ...
                              if (confirm(`Muat seluruh ${totalItemsCount} suku cadang dari semua kapal di dalam SPK ${matchedSPK.spk_number} ke dalam Form TUG 6?`)) {
                                const loadedItems = matchedSPK.vessels.flatMap(v =>
                                  v.items.map(it => {
                                    ...
                                    return {
                                      ...
                                      notes: `Diimpor dari SPK (TUG 6): ${matchedSPK.spk_number} (${v.vessel_name})`
                                    };
                                  })
                                );
                              ...
                              className="w-full py-1.5 px-3 bg-indigo-650 hover:bg-indigo-700 text-white font-bold rounded text-[10px] uppercase tracking-wider transition-all shadow-xs cursor-pointer"
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-200">
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-white border border-slate-250 p-2.5 text-xs rounded-lg text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-sans"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Catatan tambahan TUG 6, instruksi loading cargo dsb..."
                      className="w-full bg-white border border-slate-250 p-2.5 text-xs rounded-lg text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-sans"
                    />
                  </div>
                </div>
              ...
              <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl space-y-3">
                <h4 className="text-[10px] font-black font-mono uppercase tracking-widest text-slate-505 border-b border-slate-200 pb-1.5 flex justify-between items-center">
                  <span>2. PILIH SUKU CADANG (SPARE PARTS TUG 6)</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-250 rounded-lg text-xs px-3 py-2 font-mono text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                    />
                  </div>
                  ...
                  <div className="md:col-span-1">
                    <button
                      ...
                      className="w-full py-2 bg-indigo-900 hover:bg-indigo-800 text-white border border-indigo-900 rounded-lg text-xs font-mono font-bold disabled:opacity-40 select-none cursor-pointer text-center"
                    >
                      ADD
                    </button>
                  </div>
              ...
              <div className="space-y-2">
                <h4 className="text-[10px] font-black font-mono uppercase tracking-widest text-slate-500 block">
                  DAFTAR GRID ITEM TUG 6 YANG AKAN DISIMPAN
                </h4>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 text-slate-700 text-[10px] font-mono uppercase tracking-wider border-b border-slate-200">
                        ...
                        <th className="py-2.5 px-3 w-32 text-center font-black text-indigo-600 bg-indigo-50/25 border-x border-slate-200">Kebutuhan</th>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-205 flex justify-end gap-3 shrink-0">
              ...
              <button
                ...
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-505 text-white text-xs font-black uppercase rounded-lg transition-all shadow-md shadow-indigo-100 cursor-pointer flex items-center gap-1.5"
              >
                <Send className="w-3.5 h-3.5" />
                Submit Permintaan TUG 6
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 no-print animate-fade-in">
          <div className="bg-white border border-slate-300 rounded-xl shadow-2xl max-w-md w-full font-sans overflow-hidden animate-zoom-in">
            <div className="p-4 border-b border-indigo-100 bg-indigo-50/50 flex justify-between items-center">
              <div className="flex items-center gap-2 text-indigo-800 font-black font-display uppercase tracking-wider text-xs">
                <span>⚡ Sinkronisasi Register SPK (TUG 6)</span>
              </div>
              <button 
                onClick={() => {
            ...
            <div className="p-5 space-y-4">
              <div className="p-3 bg-indigo-50/60 border border-indigo-105 rounded-lg flex items-start gap-2.5">
                <span className="text-xl">✨</span>
                <div className="text-xs text-indigo-900 leading-relaxed font-semibold">
                  Sistem mendeteksi <span className="font-extrabold text-indigo-700">{pendingSyncList.length} Surat Perintah Kerja (SPK)</span> eksternal baru dari WMS yang belum memiliki form TUG 6 (Permintaan Barang) di modul ini.
                </div>
              </div>
                ...
                <div className="flex items-center justify-between mb-1.5 no-print">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-[10px] text-indigo-600 hover:text-indigo-800 font-bold disabled:opacity-50"
                    >
                      Pilih Semua
                    </button>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <span className="inline-block bg-indigo-100 text-indigo-800 font-bold text-[9px] px-1.5 py-0.5 rounded-full uppercase">
              ...
              <div className="text-xs text-slate-600 leading-relaxed bg-slate-50 p-2.5 rounded border border-slate-200/80 font-semibold">
                Suku cadang, detail kapal, pelabuhan target, dan operator akan terpetakan secara otomatis dan tersimpan dalam status <span className="text-indigo-700 font-extrabold">DRAFT</span> TUG 6.
              </div>
            </div>
            <div className="p-3.5 bg-slate-50 border-t border-slate-200 flex justify-end gap-2 text-xs">
              ...
              <button
                ...
                className={`px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg flex items-center gap-1.5 transition-all shadow-sm cursor-pointer ${
              ...
              >
                {isSyncing ? (
                  ...
                  <span>Ya, Sinkronkan TUG 6 Sekarang!</span>
                )}
              </button>
            </div>
          </div>
      ...
      {/* Batch Print ZIP Modal for TUG 6 */}
      <BatchPrintZipModal
