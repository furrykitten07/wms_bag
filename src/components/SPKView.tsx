...
export default function SPKView({
  ...
  // Details Viewing Modal, and Print Overlay states
  ...
  const handleItemChange = (vIdx: number, itemIdx: number, field: string, value: any) => {
    const updated = [...vessels];
    if (field === "spare_part_id") {
      updated[vIdx].items[itemIdx].spare_part_id = value;
    } else if (field === "qty_to_pick") {
      updated[vIdx].items[itemIdx].qty_to_pick = Math.max(1, parseInt(value) || 1);
    }
    setVessels(updated);
  };

  const handleSubmitCreateSPK = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetPort.trim()) {
      alert("Silakan isi nama pelabuhan target.");
      return;
    }

    // Clean vessels data
    const cleanedVessels = vessels.map(v => ({
      vessel_name: v.vessel_name.trim() || "Kapal Tanpa Nama",
      items: v.items.filter(i => i.spare_part_id !== "").map(i => {
        const pObj = parts.find(p => p.id === i.spare_part_id);
        return {
          spare_part_id: i.spare_part_id,
          spare_part_name: pObj?.part_name || "Unknown Part",
          part_number: pObj?.part_number || "-",
          qty_to_pick: i.qty_to_pick,
          unit: pObj?.unit || "PCS"
        };
      })
    })).filter(v => v.items.length > 0);

    if (cleanedVessels.length === 0) {
      alert("Silakan pilih minimal satu suku cadang yang valid.");
      return;
    }

    const payload: Partial<SPKWorkOrder> = {
      spk_number: `SPK/WMS/${new Date().getFullYear()}/${Math.floor(1000 + Math.random() * 9000)}`,
      target_port: targetPort,
      remarks: remarks,
      vessels: cleanedVessels,
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-slate-50 text-slate-600 text-[10px] font-mono uppercase tracking-wider border-b border-slate-200">
                <th className="p-4 py-3.5 font-bold">No. SPK / Work Order</th>
                ...
                <tr>
                  <td colSpan={6} className="p-12 text-center text-slate-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                       ...
                       <p className="text-[11px] text-slate-400">Ubah kata pencarian kata kunci Anda.</p>
                    </div>
                  </td>
                </tr>
              ) : (
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white text-slate-800 rounded-lg shadow-xl w-full max-w-3xl overflow-hidden flex flex-col border border-slate-200 my-4">
            ...
            <form onSubmit={handleSubmitCreateSPK} className="p-6 overflow-y-auto max-h-[80vh] space-y-6 text-xs text-slate-800">
                          ...
                          <div key={itemIdx} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                            ...
                            <div className="md:col-span-6">
                              <select
                                ...
                                onChange={(e) => handleItemChange(vIdx, itemIdx, "spare_part_id", e.target.value)}
                            ...
                            <div className="md:col-span-2">
                              <input
                                ...
                                onChange={(e) => handleItemChange(vIdx, itemIdx, "qty_to_pick", parseInt(e.target.value) || 1)}
