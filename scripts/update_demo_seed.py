...
def main():
    ...
    pattern = r'// --- \d+ MATERIAL REQUESTS TUG 5 GENERATOR ---[\s\S]*?(?=// --- \d+ OUTBOUND DISPATCHES TUG 8 GENERATOR ---)'
    ...
    if match:
        ...
        with open(target_file, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Successfully updated {target_file} with {len(tug5_requests)} TUG 5 and TUG 6 requests!")
    else:
